...
$sql_select2 = "SELECT * FROM engmn_user";
...
if($ban == 1) {
 $button_ban = "<button class='mob-b btn btn-sm btn-outline btn-light' style='display:inline-block; height:30px;width:100px' onclick="."unban_adm('$id')".">Разбан</button>";
 $status = "Да";
 $ban_icon = "<i class='fa fa-times-circle-o' aria-hidden='true' style='color:red; margin-top:5px' id='icon-$id'></i>";
}
if($ban == 0) {
 $button_ban = "<button class='mob-b btn btn-sm btn-outline btn-light' style='display:inline-block; height:30px;width:100px' onclick="."ban_adm('$id');".">Бан</button>";
 $status = "Нет";
 $ban_icon = "";
}
...
<td class='sorting_1' tabindex='0'><b><a href='$social' style='color: #764cfd;' target='_blank'>$stat_social</a></b></td>
...
<label class='mob-b btn btn-sm btn-outline btn-light' style='display:inline-block; height:30px;width:100px' for='modal-1' onclick="."select('$id')".">Настройки</label></center></div></td>
</tr>";
}
  ?>
  ...
  <div class="modal-wrap" aria-hidden="true" role="dialog">
    ...
    <div class="modal-dialog">
      <div class="modal-header">
        <h2>Настройки данных пользователя #<span id='userid' style='color:gray;'>1</span></h2>
		<h5>Данные могут меняться не с первого нажатия, нужно нажать на кнопку несколько раз!</h5>
        <label class="btn-close" for="modal-1" aria-hidden="true">×</label>
      </div>
      <div class="modal-body">
    <hr>
    Баланс
<input type='text' class='input-bordered col-12' id='userbal' placeholder='500.00'><br>
    Подкрутка
